...
export class ComputeStack extends cdk.Stack {
  ...
  constructor(scope: Construct, id: string, props: ComputeStackProps) {
    ...
    userData.addCommands(
      ...
      `RSA_PRIVATE_KEY=$(aws ssm get-parameter --name "/ctech-account/$ENVIRONMENT/rsa-private-key" --with-decryption --query Parameter.Value --output text --region us-east-1 2>/dev/null || echo "")`,
      ...
      `PUBLIC_KEY_KID=$(aws ssm get-parameter --name "/ctech-account/$ENVIRONMENT/public-key-kid" --query Parameter.Value --output text --region us-east-1 2>/dev/null)`,
