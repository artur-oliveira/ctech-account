...
import {api, isAxiosError, API_URL} from '@/lib/axios'
...
function LoginForm() {
  ...
  const continueURL = (() => {
    if (!rawContinue) return '/account'
    // The authorize endpoint base64-encodes the continue URL (RawURL encoding, never starts with '/').
  ...
  }, [isInitialized, accessToken, continueURL, router])
  ...
  async function handlePasskeyLogin() {
    ...
    try {
      ...
      if (result?.requires_mfa && result?.mfa_token) {
        ...
        sessionStorage.setItem('mfa_methods', JSON.stringify(result.mfa_methods ?? []))
        sessionStorage.setItem('continue_url', continueURL)
        window.location.href = `/login/mfa?continue=${encodeURIComponent(continueURL)}`
        return
      }
  ...
  return (
    <div className="space-y-4">
      ...
      <Button
        ...
        className="w-full"
        onClick={() => {
          window.location.href = `${API_URL}/v1.0/auth/google?continue=${encodeURIComponent(continueURL)}`
        }}
        disabled={loading || passkeyLoading}
      >
