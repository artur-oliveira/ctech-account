...
function ProfileForm() {
  ...
  const { data: profile, isError: profileLoadError, error: profileLoadErr, refetch: refetchProfile } = useQuery({ queryKey: ['profile'], queryFn: fetchProfile })
  ...
  if (profileLoadError) {
    return <QueryError error={profileLoadErr} onRetry={() => refetchProfile()} />
  }
...
function PasswordForm() {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <input type="text" name="username" autoComplete="username" value={profile?.email ?? ''} readOnly hidden />
      <div className="space-y-1.5">
