...
type scopeClientReconcilerStub struct {
	clientID string
	scopes   []string
	changed  bool
}

func (r *scopeClientReconcilerStub) EnsureFirstPartyPublicClientScopes(_ context.Context, clientID string, requiredScopes []string) (*oauthclient.OAuthClient, bool, error) {
	r.clientID = clientID
	r.scopes = append([]string(nil), requiredScopes...)
	return nil, r.changed, nil
}
...
func newScopeRegistryHTTP(t *testing.T, managedResource string) (*fiber.App, *scopeRegistryRepoStub, *scopeAuditRepoStub, *scopeClientReconcilerStub) {
	t.Helper()
	...
	reconciler := &scopeClientReconcilerStub{}
	h := NewScopeRegistryHandler(registry, scopeClientRepoStub{client: &oauthclient.OAuthClient{PK: oauthclient.BuildPK("scope-publisher-dfe"), ManagedResourceID: managedResource}}, reconciler, audit.NewService(audits))
	app := fiber.New()
	v1 := app.Group("/v1.0")
	...
	return app, repo, audits, reconciler
}

func TestScopeRegistryHTTPPublishAndETag(t *testing.T) {
	app, repo, audits, reconciler := newScopeRegistryHTTP(t, "dfe")
	reconciler.changed = true
	...
	if err := json.NewDecoder(resp.Body).Decode(&result); err != nil || result["changed"] != true || result["first_party_client_changed"] != true {
		t.Fatalf("response=%v err=%v", result, err)
	}
	if len(audits.events) != 2 || audits.events[0].EventType != audit.EventScopeManifestPublished || audits.events[1].EventType != audit.EventOAuthClientUpdated {
		t.Fatalf("audit events=%+v", audits.events)
	}
	if reconciler.clientID != "dfe" || len(reconciler.scopes) != 1 || reconciler.scopes[0] != "dfe:nfes:read" {
		t.Fatalf("first-party client reconciliation: %+v", reconciler)
	}
}

func TestScopeRegistryHTTPRejectsWrongBindingAndMissingPrecondition(t *testing.T) {
	app, _, _, _ := newScopeRegistryHTTP(t, "wallet")
	...
	app, _, _, _ = newScopeRegistryHTTP(t, "dfe")
