...
// GetAndDelete atomically fetches and consumes an authorization code, so two
// concurrent token requests cannot both redeem the same code (replay-safe).
func (r *Repository) GetAndDelete(ctx context.Context, codeHash string) (*AuthCode, error) {
	key := keyPrefix + codeHash
	var ac AuthCode
	if err := r.cache.GetDel(ctx, key, &ac); err != nil {
		if errors.Is(err, cache.ErrNotFound) {
			return nil, ErrNotFound
		}
		return nil, fmt.Errorf("consuming auth code: %w", err)
	}
	return &ac, nil
}
